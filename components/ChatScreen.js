...
function ChatScreen({ chat, messages }) {
  ...
  const removeConversation = () => {
    const messageRef = db
      ...
      .doc(router.query.id)
      .collection("messages");
    const userIsTypingRef = db
      .collection("chats")
      .doc(router.query.id)
      .collection("isTyping")
      .doc(user.email);
    const recipientIsTypingRef = db
      .collection("chats")
      .doc(router.query.id)
      .collection("isTyping")
      .doc(recipientEmail);
  ...
  const sendMessage = async (e) => {
    ...
    if (upload !== null) {
      ...
      if (!validImageTypes.includes(fileType)) {
        const attachmentStorageRef = firebase
          .storage()
          .ref("attachment")
          .child(upload.name);
      ...
      <Header>
        ...
        <HeaderInfo>
          ...
          {recipientSnapshot ? (
            recipient?.isOnline ? (
              ...
              <p>
                Active <TimeAgo datetime={recipient?.lastSeen?.toDate()} />
              </p>
            ) : (
        ...
        <HeaderIcons>
          {upload !== null ? (
            ...
            >
              <HighlightOffIcon
                style={{ fontSize: 25, color: "#dc2f02" }}
      ...
      <Dialog open={open} onClose={AlertHandleClose}>
        ...
        <ButtonContainer>
          <Button
            ...
            variant="outlined"
            size="large"
            style={{ color: "#dc2f02", borderColor: "#dc2f02" }}
          >
            Agree
          </Button>
